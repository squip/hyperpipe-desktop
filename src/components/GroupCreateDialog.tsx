...
function normalizeVerifiedOperatorPubkey(operatorIdentity?: GatewayOperatorIdentity | null): string {
  if (!operatorIdentity || typeof operatorIdentity !== 'object') return ''
  return typeof operatorIdentity.pubkey === 'string' ? operatorIdentity.pubkey.trim().toLowerCase() : ''
}
  ...
  return (
    <div className="flex min-w-0 items-center gap-3">
      {verifiedOperatorPubkey ? (
        ...
        <div className={cn(compact ? 'h-9 w-9' : 'h-10 w-10', 'flex shrink-0 items-center justify-center rounded-full border border-border/70 bg-muted/30 text-muted-foreground')}>
...
function SelectedGatewayDetails({
  gateway
}: {
  gateway: AuthorizedGateway
}) {
...
export default function GroupCreateDialog({
  ...
  const authorizedGateways = useMemo(() => {
    ...
    return rows
      .map((row) => {
        ...
        const operatorIdentity = row.operatorIdentity && typeof row.operatorIdentity === 'object'
  ...
  const selectedGateway = useMemo(() => {
    ...
    return (
      authorizedGateways.find((gateway) => `gateway:${gateway.gatewayId}` === gatewaySelection) || null
    )
  ...
  const matchedManualGateway = useMemo(() => {
    ...
    return authorizedGateways.find((gateway) => normalizeHttpOrigin(gateway.publicUrl) === manualOrigin) || null
            ...
            <Popover open={gatewayPickerOpen} onOpenChange={setGatewayPickerOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  <div className="min-w-0 flex-1">
                    {selectedGateway ? (
                      <GatewayOptionPreview gateway={selectedGateway} isSelected={false} compact />
